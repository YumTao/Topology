...
//		spoutConfig.forceFromStart = true;// 从头开始读
		...
		builder.setBolt(URLCOUNT_BYIP_BOLT_ID, new UrlCountCacuByIpBolt(), 1).fieldsGrouping(GET_IP_URL_BOLT_ID,
				new Fields(Const.GETIPURLBOLT_FIELD_IP));
		builder.setBolt(URLCOUNT_BOLT_ID, new UrlCountCacuBolt(), 1).fieldsGrouping(GET_IP_URL_BOLT_ID,
...
//		cluster.submitTopology(TP_ID, conf, builder.createTopology());
		...
		StormSubmitter.submitTopology(TP_ID, conf, builder.createTopology());
	}
}